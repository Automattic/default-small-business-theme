...
function business_elegant_add_ellipses_to_nav( $items, $args ) {
	if ( 'menu-1' === $args->theme_location ) :
		...
		$items .= '<span class="screen-reader-text">'. esc_html( 'More', 'business_elegant' ) . '</span>';
...
if ( ! function_exists( 'business_elegant_excerpt_more' ) ) :
    function business_elegant_excerpt_more( $more ) {
        $link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
            ...
            sprintf( esc_html__( 'Continue reading %s', 'business_elegant' ), '<span class="screen-reader-text">' . get_the_title( get_the_ID() ) . '</span>' )
            );
        return ' &hellip; ' . $link;
    }
