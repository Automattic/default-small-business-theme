...
if ( ! function_exists( 'business_professional_posted_on' ) ) :
	...
	function business_professional_posted_on() {
		...
		if ( is_single() ) {
			$posted_on = sprintf( esc_attr__( 'Posted on %1$s', 'business_professional' ),
							'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
						);
		}
		elseif ( is_sticky() ) {
			$posted_on = '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . esc_html__( 'Featured', 'business_professional' ) . '</a>';
		}
		else {
		...
		$byline = sprintf(
			...
			esc_html_x( ' by %s', 'post author', 'business_professional' ),
...
if ( ! function_exists( 'business_professional_entry_footer' ) ) :
	...
	function business_professional_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'business_professional' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'business_professional' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'business_professional' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'business_professional' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'business_professional' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'business_professional' ),
					array(
						'span' => array(
							'class' => array(),
						),
